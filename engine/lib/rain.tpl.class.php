...
class RainTPL {
     ...
     * eg. 	$t->exist('name');
     * @param type $variable
     * @return boolean true if variable exist
     */
    ...
    /**
     * Draw the template from string
     *
     * @param string $templateString  string to compile and load
     */
    function drawString($templateString) {
        //xml substitution
        $template_code = preg_replace("/<\?xml(.*?)\?>/s", "##XML\\1XML##", $templateString);
        ...
        $template_code = preg_split($tag_regexp, $templateString, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
        $compiled_code = "<?php if(!class_exists('raintpl')){exit;}?>" . $this->compileCode($template_code);
        /* Koniec kompilacji */

        $compiled_code = str_replace("?>\n", "?>\n\n", $compiled_code);
        $compiled_code = str_replace("&#039;", "'", $compiled_code);

        $cache_dir = self::$root_dir . self::$cache_dir;
        ...
        $file = $cache_dir . "/pluginTMP";

        file_put_contents($file, $compiled_code);
        include_once $file;
        unlink($file);
    }
    ...
    static function getconfig($setting = null) {
        if ($setting === null) {
            return self::$config_name_sum;
